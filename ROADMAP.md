...
Implementation: `COMPLETE (Commit 21K HA-F22 Scan-Dependent UI Rehydration Closure: (1) In app/server.py, extended /api/runtime/context with summary, unique_property_count, and scan_seconds; (2) In app/ui/index.html, rehydrated S.vaultSummary, S.uniquePropertyCount, S.lastScanSeconds; added restoreActiveScanUI() called on init when S.scanned is true to un-hide overviewScanned, populate note/property stats cards, rehydrate scope folders and controls via renderScopeControlsFromState(), and rebuild scan-dependent caches; (3) In tests/test_v12_human_acceptance_repairs.py, updated test_ha_f22_backend_runtime_context_active_and_empty and extended test_ha_f22_frontend_runtime_context_rehydration_in_node with full Overview & Scope rehydration assertions (TESTS C-H))`  
